...
BOOST_AUTO_TEST_CASE(addrman_simple)
{
    ...
    CService addr1 = ResolveService("250.1.1.1", 42003);
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 1U);
    auto addr_ret1 = addrman->Select().first;
    BOOST_CHECK_EQUAL(addr_ret1.ToStringAddrPort(), "250.1.1.1:42003");

    // Test: Does IP address deduplication work correctly.
    //  Expected dup IP should not be added.
    CService addr1_dup = ResolveService("250.1.1.1", 42003);
    ...
    CService addr2 = ResolveService("250.1.1.2", 42003);
    ...
    vAddr.emplace_back(ResolveService("250.1.1.3", 42003), NODE_NONE);
    vAddr.emplace_back(ResolveService("250.1.1.4", 42003), NODE_NONE);
...
BOOST_AUTO_TEST_CASE(addrman_ports)
{
    ...
    CService addr1_port = ResolveService("250.1.1.1", 42034);
    BOOST_CHECK(addrman->Add({CAddress(addr1_port, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 2U);
    auto addr_ret2 = addrman->Select().first;
    BOOST_CHECK(addr_ret2.ToStringAddrPort() == "250.1.1.1:42003" || addr_ret2.ToStringAddrPort() == "250.1.1.1:42034");
    ...
    BOOST_CHECK_EQUAL(addr_ret3.ToStringAddrPort(), "250.1.1.1:42003");
}
...
BOOST_AUTO_TEST_CASE(addrman_select)
{
    ...
    CService addr2 = ResolveService("250.3.1.1", 42003);
    ...
    BOOST_CHECK(addrman->Add({CAddress(addr7, NODE_NONE)}, ResolveService("250.1.1.3", 42003)));
...
BOOST_AUTO_TEST_CASE(addrman_select_special)
{
    ...
    // add ipv4 address to the new table
    CService addr1 = ResolveService("250.1.1.3", 42003);
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
...
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CAddress addr{CAddress(ResolveService("253.3.3.3", 42003), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(addrman_getaddr)
{
    ...
    CAddress addr4 = CAddress(ResolveService("252.253.3.4", 42003), NODE_NONE);
    addr4.nTime = Now<NodeSeconds>();
    CAddress addr5 = CAddress(ResolveService("252.254.4.5", 42003), NODE_NONE);
    ...
    BOOST_CHECK_EQUAL(addrman->Size(), 2005U);
}
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_tried_bucket_legacy)
{
    ...
    BOOST_CHECK_EQUAL(info1.GetTriedBucket(nKey1, EMPTY_NETGROUPMAN), 77);
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_tried_bucket)
{
    ...
    BOOST_CHECK_EQUAL(info1.GetTriedBucket(nKey1, ngm_asmap), 169);
...
BOOST_AUTO_TEST_CASE(load_addrman)
{
    ...
    const std::optional<CService> source{Lookup("252.5.1.1", 42003, false)};
...
BOOST_AUTO_TEST_CASE(addrman_update_address)
{
    ...
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CNetAddr source{ResolveIP("252.2.2.2")};
    CAddress addr{CAddress(ResolveService("250.1.1.1", 42003), NODE_NONE)};
    ...
    CAddress addr_diff_port{CAddress(ResolveService("250.1.1.1", 42034), NODE_NONE)};
    ...
    CAddress addr_v2{CAddress(ResolveService("250.1.1.1", 42003), NODE_P2P_V2)};
...
BOOST_AUTO_TEST_CASE(addrman_size)
{
    ...
    const CAddress addr1{ResolveService("250.1.1.1", 42003), NODE_NONE};
    BOOST_CHECK(addrman->Add({addr1}, source));
    BOOST_CHECK(addrman->Good(addr1));
    const CAddress addr2{ResolveService("250.1.1.2", 42003), NODE_NONE};
