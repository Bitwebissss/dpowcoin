                                            ...
                                            bool via_compact_block)
{
    ...
    bool have_headers_sync = false;
    {
        ...
        if (headers.empty()) {
            ...
            //
            // NOTE: this renews m_presync_liveness_timeout only, a dedicated field
            // read exclusively by ConsiderEviction's PRESYNC bypass. It deliberately
            // does NOT touch m_headers_sync_timeout, which remains the stock,
            // untouched signal for the original headers-download-stall check.
    ...
    // Extend the dedicated presync-liveness deadline by HEADERS_PRESYNC_RENEWAL_TIMEOUT
    // so the peer has time to deliver the next batch. This mirrors the renewal in
    // the PRESYNC branch above; together they implement the full sliding-window
    // mechanism for both phases of low-work headers sync.
    //
    // This only renews m_presync_liveness_timeout — it must NOT touch
    // m_headers_sync_timeout, which is reserved for the unrelated stock
    // headers-download-stall check and for ConsiderEviction's normal,
    // non-presync chain-sync-timeout logic.
