...
`dpowcoin rpc` can also be substituted for `dpowcoin-cli`.
...
`dpowcoin node` or `dpowcoin gui` can also be substituted for `dpowcoind`.
...
and open port 42003 on your firewall (or use port mapping, i.e., `-natpmp`).
...
- Operating a node that listens on multiple networks (e.g. IPv4 and Tor) can help
  strengthen the Dpowcoin network, as nodes in this configuration (i.e. bridge nodes) increase
